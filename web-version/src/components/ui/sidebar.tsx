...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const [width, setWidthState] = React.useState(() => {
    ...
    return Number.isFinite(stored) ? clampWidth(stored) : SIDEBAR_DEFAULT_WIDTH;
  });
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(
  ({ className, ...props }, ref) => {
    const { isMobile, open, setOpen, toggleSidebar, width, setWidth } = useSidebar();
    const dragState = React.useRef<{ pointerId: number; startX: number; startWidth: number; moved: boolean } | null>(null);
    ...
    const handlePointerDown = (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      const handlePointerMove = (moveEvent: PointerEvent) => {
        ...
        dragState.current.moved = dragState.current.moved || Math.abs(delta) > 3;
